...
/* AGGIORNAMENTO TABELLE DATABASE AL 19/08/2014
  Aggiornamento tabella articoli per l'inserimento dell'ordine di stampa catalogo

 */

echo "<h4>Inizio aggiornamento archivi versione 39</h4>\n";


$query = "ALTER TABLE articoli ADD ordine_cat VARCHAR( 30 ) CHARACTER SET utf8 COLLATE utf8_general_ci NULL AFTER art_alternativo";

$conn->exec($query);

if ($conn->errorCode() == "00000") // ... tutto ok
{
    echo "Alterazione tabella articoli agg. 39 punto 1.........  riuscita perfettamente <br>";
    $_num++;
    $fine = 1;
}
else
{
    echo "Alterazione tabella agg. 39 punto 1 $db_nomedb<br>";

    $errorinfo = $conn->errorInfo();
    echo "Errore numero $errorinfo[1] tipo $errorinfo[2] <br/>"; // stringa con l' errore
    $_errori['descrizione'] = $errorinfo[2];
    $_errori['files'] = "39.sql.db.php";
...
$query = "ALTER TABLE articoli ADD egpz CHAR( 2 ) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT 'NO' AFTER ordine_cat";

$conn->exec($query);

if ($conn->errorCode() == "00000") // ... tutto ok
{
    echo "Alterazione tabella articoli agg. 39 punto 2.........  riuscita perfettamente <br>";
    $_num++;
    $fine = 1;
}
else
{
    echo "Alterazione tabella agg. 39 punto 2 $db_nomedb<br>";
...
else
{
    ...
    echo "Aggiornamento tabella versione..... 39 del 19/08/2014 COMPLETATO !<br>";
