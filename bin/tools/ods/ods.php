...
<manifest:manifest xmlns:manifest="urn:oasis:names:tc:opendocument:xmlns:manifest:1.0">
 ...
 <manifest:file-entry manifest:media-type="text/xml" manifest:full-path="settings.xml"/>
	...
	function readCell($sheet,$row,$cell) {
		$value['uno'] = $this->sheets[$sheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'];
		$value['due'] = $this->sheets[$sheet]['rows'][$row][$cell]['value'];
		
		return $value;
	}
	
	function readRow($sheet,$row,$cell) {
		//$value['uno'] = $this->sheets[$sheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'];
		$value = $this->sheets[$sheet]['rows'][$row];
		
		return $value;
	}
	
	function readSheet($_cosa, $sheet) {
		//$value['uno'] = $this->sheets[$sheet]['rows'][$row][$cell]['attrs']['OFFICE:VALUE'];
		if($_cosa == "numero")
		{
		  $value = count($this->sheets[$sheet]['rows']);
		}
		else
		{
		  $value = $this->sheets[$sheet]['rows'];
		}
