...
// ora provo a cercare sull'anagrafica fornitori una corrispondenza della partiva iva
	...
	$query = sprintf( "select * from fornitori where piva=\"%s\"", $_pivacli);
	$res = mysql_query( $query, $conn );
...
// ora cerco il tutto e lo metto su video..
	...
	$query = sprintf(" select sum(qtacarico) AS carico, sum(qtascarico) as scarico, sum(qtascarico) - sum(qtacarico) AS differenza, descrizione, magazzino.articolo from magazzino INNER JOIN articoli ON magazzino. articolo=articoli.articolo where catmer='IMBALLI' and (tut='f' and utente=\"%s\" OR tut='c' and utente=\"%s\") GROUP BY magazzino.articolo ", $_codicefor, $_codicecli );
...
echo "<br>";
	...
	$query = sprintf("select sum(qtacarico) AS carico, sum(qtascarico) as scarico, sum(qtascarico) - sum(qtacarico) AS differenza, descrizione, magastorico.articolo from magastorico INNER JOIN articoli ON magastorico. articolo=articoli.articolo where catmer='IMBALLI' and (tut='f' and utente=\"%s\" OR tut='c' and utente=\"%s\" ) GROUP BY magastorico.articolo", $_codicefor, $_codicecli);
