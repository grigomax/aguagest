...
elseif ($_tut == "singolo")
{
    $query = sprintf("select * from $_dbdoc[testacalce] INNER JOIN clienti ON $_dbdoc[testacalce].utente = clienti.codice where clienti.codice = '$_utente' AND status != 'evaso' order by ragsoc, anno, suffix, ndoc");
}
elseif ($_tut == "sing_fornitore")
{
    $query = sprintf("select * from $_dbdoc[testacalce] INNER JOIN fornitori ON $_dbdoc[testacalce].utente = fornitori.codice where fornitori.codice = '$_utente' AND status != 'evaso' order by ragsoc, anno, suffix, ndoc");
}
else
{

    $query = sprintf("select * from $_dbdoc[testacalce] INNER JOIN clienti ON $_dbdoc[testacalce].utente = clienti.codice where status != 'evaso' order by ragsoc, anno, suffix, ndoc");
}
...
// Tutto procede a meraviglia...
    ...
    foreach ($result as $dati)
    {
        ...
        $_barcode = "";
        $_barcode = ".$dati[anno].$dati[suffix].$dati[ndoc]";
...
//ora per ogni orndine facciamo apparire le righe con lo status..
        ...
        $query = "select * from $_dbdoc[dettaglio] where anno='$dati[anno]' and suffix='$dati[suffix]' and ndoc='$dati[ndoc]' order by anno, suffix, ndoc, rigo";
       // echo "<br>$query";
