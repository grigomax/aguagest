...
//mi prendo l'elenco dei mesi..
	...
	if ($_azione == "liquida")
	{
		...
		$liquid = tabella_liquid_iva_periodica("singolo", $_anno, $_periodo, $_parametri);
		...
		$_parametri['desc_conto'] = piano_conti($CONTO_IVA_ERARIO, "descsingola");
		$_parametri['dare'] = $liquid['val_liquid'];

		//inseriamo la prima scrittura nella prima nota...
		$_result = tabella_primanota("Inserisci_singolo", $id, $_anno_reg, $_nreg, "ST", $_testo, $_datareg, $_datareg, $_parametri, $_percorso);
		...
		$_banca = tabella_banche("singolo", $_POST['banca'], $_abi, $_cab, $_parametri);
		...
		if ($_return['errori']['errore'] != "")
		{
			echo "<h2>Errore nell'Aggiornamento</h2>\n";
			echo $_return[errori][descrizione];
			echo "procedura bloccata\n";
			exit;
		}
...
//dividiamo le funzioni...
		if ($_azione == "consolida")
		{
			...
			if ($_POST['periodo'] != "13")
			{
				...
				//inseriamo la prima scrittura nella prima nota...
				$_result = tabella_primanota("Inserisci_singolo", $id, $_POST['anno'], $_nreg, "ST", $_testo, $_datareg, $_datareg, $_parametri, $_percorso);
				...
				$_parametri['desc_conto'] = piano_conti($CONTO_IVA_ACQUISTI, "descsingola");
				...
				$_parametri['desc_conto'] = piano_conti($CONTO_IVA_VENDITE, "descsingola");
				...
				while ($dati = mysql_fetch_array($result))
				{
					...
					{
						echo "errore nell'aggiornamento $query<br>";
						echo mysql_error();
					}
				}
				...
				while ($dati = mysql_fetch_array($result))
				{
					...
					if (mysql_query($query, $conn) != 1)
					{
						echo "errore nell'aggirnamento $query<br>";
						echo mysql_error();
					}
				}
