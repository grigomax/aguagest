...
//recupero tutti POST se il GET non c'è
	...
	if ($_POST['diretto'] != "")
	{
		...
		{
			$_codconto = $_POST['diretto'];
			$query = "SELECT codconto, descrizione, tipo_cf FROM piano_conti WHERE codconto = '$_codconto'";
			$res = mysql_query($query, $conn);
			$dati = mysql_fetch_array($res);
	...
	if ($res = mysql_query($query, $conn))
	{
		...
		if (mysql_num_rows($res))
		{
			...
			while ($dati = mysql_fetch_array($res))
			{
				...
				printf("<td width=\"30\" align=\"center\" class=\"tabella_elenco\">%s</span></td>\n", $dati['nproto']);
				...
				printf("<td width=\"70\" align=\"right\"class=\"tabella_elenco\">%s $_scritta_p</span></td>\n", number_format($_saldo, '2'));
