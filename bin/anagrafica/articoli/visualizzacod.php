...
// CAMPO selezione categoria merceologica -------------------------------------------
	...
	foreach ($res AS $dati)
	{
		...
		printf("<tr><td align=\"left\">%s</td><td><a href=\"../../vendite/docubase/visualizzadoc.php?tdoc=ddtacq&anno=$_annodoc&ndoc=%s\">%s</a></td><td colspan=\"2\" align=\"left\">%s</td><td align=\"center\">%s</td><td>%s</td><td>%s</td></tr>", $dati['datareg'], $dati['ndoc'], $dati['ndoc'], $dati['ragsoc'], $dati['qtacarico'], $dati['valoreacq'], number_format(($_nettoacq), 2));
	}
