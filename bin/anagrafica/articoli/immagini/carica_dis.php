...
if ($_SESSION['user']['anagrafiche'] > "1")
{
	...
	if ($_azione == "Elimina")
	{
		...
		unlink("../../../../imm-art/disegni/$_POST[file]");
...
//controlliamo che il file rispetti le dimensioni impostate
			if ($_FILES["file"]["size"] < "16777216")
			{
				...
				else
				{
					...
					move_uploaded_file($_FILES["file"]["tmp_name"], "../../../../imm-art/disegni/" . $_FILES["file"]["name"]);
...
// Ottengo le informazioni sull'immagine originale
			list($width, $height, $type, $attr) = getimagesize('../../../../imm-art/disegni/' . $_FILES['file']['name']);
...
//
			if (($width > "500") OR ($height > "500"))
			{
				...
				$source = imagecreatefromjpeg('../../../../imm-art/disegni/' . $_FILES['file']['name']);
				imagecopyresized($thumb, $source, 0, 0, 0, 0, $_width, $_height, $width, $height);

				// Salvo l'immagine ridimensionata
				imagejpeg($thumb, '../../../../imm-art/disegni/' . $_FILES['file']['name'], 90);
			}
		...
		else
		{
			...
			rename("../../../../imm-art/disegni/$_POST[orfile]", "../../../../imm-art/disegni/$_POST[file]");

			$_file = $_POST['file'];

			echo "File Rinominato con Successo";
		}
		echo "<form action=\"carica_dis.php\" method=\"post\">\n";
...
//visualizziamo l'immagine e chiediamo se la si vuole cancellare rinominare oppure se è tutto ok..
		...
		echo "<p style='text-align:center;'><img src=\"../../../../imm-art/disegni/$_file\" >";
